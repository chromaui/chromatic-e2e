...
export class ChromaticReporter implements Reporter {
  ...
  private constructor(
    ...
    private entityToSnapshotCount = new Map<TestCase['id'] | TestModule['id'], number>()
  ) {}
  ...
  onTestCaseResult(testCase: TestCase) {
    ...
    this.logSnapshots({
      count: this.entityToSnapshotCount.get(testCase.id) ?? 0,
      indentation: 3,
  ...
  onTestModuleEnd(testModule: TestModule) {
    ...
    this.logSnapshots({
      count: this.entityToSnapshotCount.get(testModule.id) ?? 0,
  ...
  onTestRunEnd() {
    const snapshotCount = [...this.entityToSnapshotCount.values()].reduce(sum, 0);
    this.entityToSnapshotCount.clear();
  ...
  private _onSnapshot(test: TestCase) {
    // Verbose reporter logs outputs of each test cases, others after each test module:
    const key = this.options.builtInReporter === 'verbose' ? test.id : test.module.id;

    const entityToSnapshotCount = this.entityToSnapshotCount.get(key) ?? 0;
    this.entityToSnapshotCount.set(key, entityToSnapshotCount + 1);
  }
}
...
function sum(total: number, acc: number) {
  return total + acc;
}
